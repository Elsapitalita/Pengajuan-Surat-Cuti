...
public class BerandaAdminController implements Initializable {
    ...
    void konfirmasi(ActionEvent event) throws IOException {
        int selected = pengajuan_tbl.getSelectionModel().getSelectedIndex();
        int selected_id = 0;
        if(selected >= 0) {
            if(pengajuan_tbl.getSelectionModel().getSelectedItem() != null) {
                Pengajuan pengajuan = pengajuan_tbl.getSelectionModel().getSelectedItem();
                selected_id = pengajuan.getId();
            }
            
            Session ss = util.HibernateUtil.getSessionFactory().openSession();
            ss.beginTransaction();
            Query query = ss.createQuery("UPDATE Pengajuan SET status='Dikonfirmasi' where id ="+selected_id);
            query.executeUpdate();
